...
const _excluded = ["discordClientId", "redirectUri", "responseType", "scopes", "debug", "backgroundColor", "size", "label", "onClick", "newTab"],
  _excluded2 = ["primary", "backgroundColor", "size", "label", "newTab"];
...
const DiscordLogin = _ref => {
  ...
  }))), /*#__PURE__*/_react.default.createElement("span", null, label && label != '' ? label : 'Sign in with Discord'));
};
...
DiscordButtonStory.propTypes = {
  ...
  /**
   * Provides some helpful logs to the console
   */
  debug: _propTypes.default.bool,
  /**
   * Whether to open the oAuth box in a new tab
   */
  newTab: _propTypes.default.bool,
  /**
   * Your Discord Client Id. Find yours at: https://discord.com/developers/applications > Application > OAuth2
   */
  discordClientId: _propTypes.default.string,
  /**
   * URL to redirect too after success or failure of user auth. Will attach discord code to be used in follow up
   * api requests as the query parameter `code`. Has to match one of the URLs
   * under https://discord.com/developers/applications > Application > OAuth2
   * If doing server auth Authorization flow, redirect to your server endpoint handling the rest of the login
   * Otherwise you can redirect to your and handle there
   */
  redirectUri: _propTypes.default.string,
  /**
   * The response type from the Discord oAuth. Defaults to `code`
   */
  responseType: _propTypes.default.string,
  /**
   * String array of scopes you need access to. Identify should always be applied, and if you're doing the
   * Authorization flow you'll probably need `identify` as well. Default is ['identify', 'email']
   */
  newTab: [_propTypes.default.string]
};
